...
function[time, result, diff_star, nabla] = computing_qr(dim)
    [A, b] = data_prep(0, dim);
    x_star =  A\b;
...
function[time, result, diff_star, nabla] = computing_cg(dim, threshold)
...
time = 0;
for i=1:1000
    [x_cg, ~, it, t] = custom_conjgrad(sym_A, sym_b, b, threshold*5, 440, 1);
    time = time + t(it);
end
time = time/1000;
